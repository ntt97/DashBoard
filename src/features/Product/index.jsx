...
function Product(props) {
  ...
  useEffect(() => {
    ...
    async function fectchData() {
      await axios
        .get(
          `${PREFIX_URL_API}/products/currentPage=${pagination.currentPage}&limitPerPage=${pagination.limitPerPage}`
        )
        .then((response) => {
  ...
  const onReLoadAdd = () => {
    ...
    if (
      ...
      pagination.limitPerPage === dataCurrentPage.length
    ) {
  ...
  const onChangeTextSearch = async (e) => {
    ...
    const newData = dataCurrentPage.filter((pro) =>
      JSON.stringify(pro).toLowerCase().includes(textSearch.toLowerCase())
    );
      ...
      >
        <div className="alert-section">
          <section className="alert-section__width">
            <Alert className={`${alert.color} `}>{alert.message}</Alert>
          </section>
        </div>
      </CSSTransition>
                ...
                <div className="mb-3 float-right col-4">
                  <InputGroup>
                    ...
                    <InputGroupAddon addonType="append">
                      <Button color="secondary" onClick={clearTextSearch}>
                        Clear
                      </Button>
                    </InputGroupAddon>
                  </InputGroup>
